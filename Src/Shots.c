...
void shot_add(SHOT_ARGS *args, char x_direction, char y_direction, char wave_range, char wave_type)
{
	SHOT *s;
	short i = first_empty_shot;
	short x, y;
...
void shot_create(char player, char remove, short x, short y, char direction, short combo)
{
	...
	if(pattern == COMBO_NORMAL)
		shot_add(&args, 0, 0, 0, WAVE_ONCE);
	else if(pattern == COMBO_WAVE)
		shot_add(&args, x_dir, y_dir, wave_range, WAVE_CONT);
	else if(pattern == COMBO_SPREAD) {
		shot_add(&args, x_dir, y_dir, wave_range, WAVE_ONCE);
	...
	for(i = first_shot ; i >= 0 ; i = next) {
		...
		if(glbs->shots[i].decay_counter) {
			...
			if(glbs->shots[i].decay_counter == 0) {
				glbs->shots[i].y_direction = 6;
				glbs->shots[i].wave_range = 128;
				glbs->shots[i].wave_type = WAVE_DECAY;
			}
		}
		...
		if(glbs->shots[i].x_direction != 0) {
			...
			if(ABS(glbs->shots[i].delta_x) == glbs->shots[i].wave_range) {
				if(glbs->shots[i].wave_type == WAVE_DECAY) glbs->shots[i].x_direction = 0;
				else glbs->shots[i].x_direction *= -1;
			}
		}
		if(glbs->shots[i].x_direction && glbs->shots[i].delta_x == 0 && glbs->shots[i].wave_type == WAVE_ONCE)
			glbs->shots[i].x_direction = 0;

		if(glbs->shots[i].y_direction != 0) {
			glbs->shots[i].delta_y += glbs->shots[i].y_direction;
			if(ABS(glbs->shots[i].delta_y) == glbs->shots[i].wave_range) {
				if(glbs->shots[i].wave_type == WAVE_DECAY) glbs->shots[i].y_direction = 0;
				else glbs->shots[i].y_direction *= -1;
			}
		}
		if(glbs->shots[i].y_direction && glbs->shots[i].delta_y == 0 && glbs->shots[i].wave_type == WAVE_ONCE)
			glbs->shots[i].y_direction = 0;
		...
		ok = FALSE; // == TRUE will mean deletion

		if(data >= SOLID || (!glbs->combo_data[glbs->shots[i].combo].wave &&
		block_test(sx+sw/2, sy+sh/2, 1, 1, 0, 0))) {
			if(glbs->shots[i].player) {
		...
		glbs->shot_gfx + glbs->shots[i].header->offset, player_size_mask)) {
			if(player_hurt(glbs->combo_data[glbs->shots[i].combo].damage, glbs->shots[i].x, glbs->shots[i].y, FALSE))
				ok = TRUE;
		}
		if(ok) shot_delete(i);
	}
