...
char entity_check_size(ENTITY *e, ENTITY_SIZE *size)
{
	...
	return !block_test(e->x + size->x_off, e->y + size->y_off, size->width, size->hieght, 0, 0);
}
...
void entity_reset_slope(ENTITY *e)
{
	if(e->gravity == DOWN) {
		...
		e->slope_left_down = SLOPE_LEFT_DOWN;
		e->slope_right_down = SLOPE_RIGHT_DOWN;
	} else if(e->gravity == UP) {
		e->slope_left_up = SLOPE_LEFT_DOWN;
		e->slope_right_up = SLOPE_RIGHT_DOWN;
		e->slope_left_down = SLOPE_LEFT_UP;
		e->slope_right_down = SLOPE_RIGHT_UP;
	} else if(e->gravity == RIGHT) {
		e->slope_left_up = SLOPE_RIGHT_DOWN;
		e->slope_right_up = SLOPE_RIGHT_UP;
		e->slope_left_down = SLOPE_LEFT_DOWN;
		e->slope_right_down = SLOPE_LEFT_UP;
	} else if(e->gravity == LEFT) {
		e->slope_left_up = SLOPE_LEFT_DOWN;
		e->slope_right_up = SLOPE_LEFT_UP;
		e->slope_left_down = SLOPE_RIGHT_DOWN;
		e->slope_right_down = SLOPE_RIGHT_UP;
	}
}
...
void entity_draw(ENTITY *e, unsigned char *gfx, char flash, char use_offset, char wmask)
{
	...
	short x = e->x + header->x_offset - glbs->camera.x;
	short y = e->y - glbs->camera.y;
	...
	/*if(1) {
		SCR_RECT r = (SCR_RECT){{e->x + e->x_off - glbs->camera.x, e->y + e->y_off - glbs->camera.y,
			e->x + e->x_off + e->width - 1 - glbs->camera.x, e->y + e->y_off + e->hieght - 1 - glbs->camera.y}};
		SCR_RECT clip = (SCR_RECT){{0, 0, 239, 127}};
		PortSet(glbs->light_buffer, 239, 127);
		ScrRectFill(&r, &clip, A_NORMAL);
		PortSet(glbs->dark_buffer, 239, 127);
		ScrRectFill(&r, &clip, A_NORMAL);
	}*/
}
...
/*void check_slope_up(ENTITY *e, short tile_x, short tile_y, char x_dir)
{
	...
	if((x_dir < 0 && data == e->slope_left_up) || (x_dir > 0 && data == e->slope_right_up))
		hit_slope_up = TRUE;
	if(e->water_walking && hit_slope_up) { //Kinda a hack here...
		short yoff = e->y + e->y_off + e->hieght - tile_y * 12;
		short xoff;
		if(x_dir < 0) xoff = tile_x * 12 + 11 - (e->x + e->x_off - 1) + 1;
		else xoff = e->x + e->x_off + e->width + 1 - tile_x * 12;
		if(xoff + yoff <= 12) hit_slope_up = FALSE;
	}
}*/

char entity_check_tile_horiz(ENTITY *e, short tile_x, short tile_y, char x_dir)
{
	char data = get_prop_data(tile_x, tile_y);
	short y_off = 0;
	short x_off = 0;
	...
	if(e->damage_type != 0 && data >= BREAK_SHOOT && data <= BREAK_SHOOT_BOMB_SWITCH)
		return !hole_create(tile_x, tile_y, e->damage_type);
	else if(data < SLOPE_RIGHT_UP) return TRUE;

	//if(e->gravity == UP || e->gravity == DOWN) return FALSE;
	
	if(data == SLOPE_RIGHT_UP || data == SLOPE_LEFT_UP) {
		if(tile_y1 > tile_y) y_off = 11;//return TRUE;
		else y_off = tile_yoff1;
	} else {
		if(tile_y0 < tile_y) y_off = 11;//return TRUE;
		else y_off = 11 - tile_yoff0;
	}

	if(data == SLOPE_RIGHT_UP || data == SLOPE_RIGHT_DOWN) {
		if(tile_x1 > tile_x) x_off = 11;
		else x_off = tile_xoff1;
	} else {
		if(tile_x0 < tile_x) x_off = 11;
		else x_off = 11 - tile_xoff0;
	}
	
	//if(x_off+y_off>=11){clrscr();printf("%d %d",x_off,y_off);pause();}
	if(x_off + y_off >= 11) {
		if((e->gravity == UP || e->gravity == DOWN) &&
		((x_dir > 0 && data == e->slope_right_up) || (x_dir < 0 && data == e->slope_left_up))) {
			hit_slope_up = TRUE;
			return FALSE;
		} else return TRUE;
	} else return FALSE;
...
char entity_check_tile_vert(ENTITY *e, short tile_x, short tile_y, char y_dir)
{
	...
	if((e->damage_type & DAMAGE_STAND) &&
		(data >= BREAK_STAND_REFORM && data <= BREAK_STAND_CHAIN) && y_dir == 1)
	...
	//if(e->gravity == LEFT || e->gravity == RIGHT) return FALSE;
	...
	if(x_off + y_off >= 11) {
		if((e->gravity == LEFT || e->gravity == RIGHT) &&
		((y_dir > 0 && data == e->slope_right_up) || (y_dir < 0 && data == e->slope_left_up))) {
			hit_slope_up = TRUE;
			return FALSE;
		} else return TRUE;
	} else return FALSE;
...
char entity_check_move(ENTITY *e, char x_dir, char y_dir)
{
	char move_ok = TRUE;
	short tile_below;
	short x0 = 0, y0 = 0;
	...
	if(block_test(e->x + e->x_off + x_dir, e->y + e->y_off + y_dir, e->width, e->hieght, x_dir, y_dir))
		return FALSE;

	if(x_dir > 0) {
		tile_xoff0++; tile_xoff1++;
		if(tile_xoff0 == 12) {
			tile_xoff0 = 0;
			tile_x0++;
		}
	...
	if(y_dir > 0) {
		if(e->water_walking && e->y + e->y_off + e->hieght == glbs->water.top) return FALSE;
		tile_yoff0++; tile_yoff1++;
		if(tile_yoff0 == 12) {
			tile_yoff0 = 0;
			tile_y0++;
		}
	...
	if(x_dir) {
		for(y = tile_y0 ; y <= tile_y1 ; y++)
			if(entity_check_tile_horiz(e, x0, y, x_dir))
				move_ok = FALSE;

		if(e->gravity == DOWN) {
			tile_below = (e->y + y_dir + e->y_off + e->hieght) / 12;
			//check_slope_up(e, x0, tile_y1, x_dir);
			if(x_dir > 0) x0 = e->tile_x0;
			else x0 = e->tile_x1;
			check_slope_down(e, x0, tile_below, x_dir);
		} else if(e->gravity == UP) {
			tile_below = (e->y + y_dir + e->y_off - 1) / 12;
			//check_slope_up(e, x0, tile_y0, x_dir);
			if(x_dir > 0) x0 = e->tile_x0;
			else x0 = e->tile_x1;
			check_slope_down(e, x0, tile_below, x_dir);
		}
	...
	if(y_dir) {
		for(x = tile_x0 ; x <= tile_x1 ; x++)
			if(entity_check_tile_vert(e, x, y0, y_dir))
				move_ok = FALSE;

		if(e->gravity == RIGHT) {
			tile_below = (e->x + x_dir + e->x_off + e->width) / 12;
			//check_slope_up(e, tile_x1, y0, y_dir);
			if(y_dir > 0) y0 = e->tile_y0;
			else y0 = e->tile_y1;
			check_slope_down(e, tile_below, y0, y_dir);
		} else if(e->gravity == LEFT) {
			tile_below = (e->x + x_dir + e->x_off - 1) / 12;
			//check_slope_up(e, tile_x0, y0, y_dir);
			if(y_dir > 0) y0 = e->tile_y0;
			else y0 = e->tile_y1;
			check_slope_down(e, tile_below, y0, y_dir);
		}
...
char entity_check_direction(ENTITY *e, char direction)
{
	...
	return entity_check_move(e, x_dir, y_dir);
}
...
char entity_move(ENTITY *e, char direction, short speed)
{
	...
	else return entity_move_internal(e, x_dir, y_dir, speed);
}
...
char entity_move_internal(ENTITY *e, char x_dir, char y_dir, short speed)
{
	...
	while(e->x_speed_counter >= 100) {
		...
		if(entity_check_move(e, xdir, 0)) {
			if(hit_slope_up && y_dir == 0) {
				if(e->gravity == DOWN) ydir = -1;
				else if(e->gravity == UP) ydir = 1;
				slope = TRUE;
			} else if(hit_slope_down && y_dir == 0 &&
			(!e->water_walking || e->y + e->y_off + e->hieght != glbs->water.top)) {
				if(e->gravity == DOWN) ydir = 1;
				else if(e->gravity == UP) ydir = -1;
				slope = TRUE;
			} else slope = FALSE;
			if(slope && entity_check_move(e, xdir, ydir)) {
				e->y += ydir;
				e->y_moved += ydir;
				e->tile_y0 = tile_y0;
				e->tile_y1 = tile_y1;
...
char entity_move_simple(ENTITY *e, char x_dir, char y_dir, short speed)
{
	...
	return (data < SOLID || data >= SLOPE_RIGHT_UP);
}
