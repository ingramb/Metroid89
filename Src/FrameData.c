...
void _main(void)
{
	FILE *file = fopen("fdata", "wb");
	PLAYER_ACTION act;
	short i;
	short j;
	
	for(i = 0 ; i < ACT_END_OF_LIST ; i++) {
		act.duration = player_action[i].duration;
		act.frame_reset = player_action[i].frame_reset;
		act.overide_action = player_action[i].overide_action;
		act.change_direction = player_action[i].change_direction;
		act.input_disabled = player_action[i].input_disabled;
		act.type = player_action[i].type;
		act.size = player_action[i].size;
		act.next_action = player_action[i].next_action;
		for(j = 0 ; j < 6 ; j++) act.animation[j] = (void *)player_action[i].animation[j] - (void*)&face_front;
		fwrite(&act, sizeof(PLAYER_ACTION), 1, file);
	}
	
	fwrite((void *)(&top) + 2, (long)((void *)(&bottom) - (void *)(&top) - 2), 1, file);
